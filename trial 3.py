...
class VoltageSOCAnalyzer(QMainWindow):
    ...
    def merge_datasets(self):
        ...
        try:
            ...
            merged = pd.concat([self.dataset1, self.dataset2], ignore_index=True)
            
            # Remove duplicates based on SOC values (keep first occurrence)
            merged = merged.drop_duplicates(subset=['SOC'], keep='first')
            
            return merged
            
        except Exception as e:
            QMessageBox.critical(self, "Merge Error", f"Error merging datasets:\n{str(e)}")
            return None
    
    def charging_analysis(self):
        """Perform charging analysis (ascending SOC order)."""
        try:
            ...
            QMessageBox.information(
                ...
                f"Merged dataset contains {len(merged_data)} data points.\n"
                f"Data sorted in ascending SOC order for charging analysis."
            )
    ...
    def discharging_analysis(self):
        """Perform discharging analysis (descending SOC order)."""
        try:
            merged_data = self.merge_datasets()
            if merged_data is None:
                return
            ...
            QMessageBox.information(
                ...
                f"Merged dataset contains {len(merged_data)} data points.\n"
                f"Data sorted in descending SOC order for discharging analysis."
            )
